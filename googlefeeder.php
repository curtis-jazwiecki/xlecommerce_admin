...
define('FEEDNAME', GF_OUT_FILE);       //from your googlebase account

define('DOMAIN_NAME', GF_DOMAIN_NAME); //your correct domain name (don't include www unless it is used but do include the shops directory)

define('FTP_USERNAME', GF_FTP_USERNAME); //created from within your googlebase account

define('FTP_PASSWORD', GF_FTP_PASSWORD); //created from within your googlebase account
...
$destination_file = GF_DESTINATION_FILE;

$source_file = $OutFile;

$imageURL = GF_IMAGE_URL;
...
if (OPTIONS_ENABLED_GOOGLE_PRODUCT_CATEGORY == 1 && OPTIONS_GOOGLE_PRODUCT_CATEGORY == 'db') {
   ...
   //$extraFields .= ' products.google_product_category as google_category, ';
   $extraFields .= ' products.google_category_path as google_category, ';
...
WHERE products.google_category_id>0 and products.products_id=products_description.products_id
...
while( $row = mysql_fetch_object( $result ) ) {
   ...
   if (OPTIONS_IGNORE_PRODUCT_ZERO < 1 && $row->quantity < 1 && $row->prodStatus == 1) {$statsArray['qty']++; $showQtyZero = true; } //record for warning
	...
	///test for unique product identifiers DWD - 9-8-11

	if($row->mpn == NULL && $row->upc == NULL) continue;

	if($row->upc == NULL && $row->brand == NULL) continue;

	if($row->mpn == NULL && $row->brand == NULL) continue;

	if(strlen($row->description)< 15) continue;
