...
function updateOrdersField(field, value) {
		...
		var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=update_order_field&oID=<?php echo $_GET['oID']; ?>&field=" + field + "&new_value=" + value;
		...
		xmlHttp.onreadystatechange=
                function(){
                    ...
                    if(xmlHttp.status==200){
                        rewriteDiv(xmlHttp.responseText, 'orders')
                        //BOF:range manager
                        if (field=='customers_telephone' && value!=''){
                            set_customer_details_by_telephone();
                        } else if (field='customers_email_address' && value!=''){
                            set_customer_details_by_email();
                        }
                        //EOF:range manager
                    }
                };
...
//BOF:range manager
function set_customer_details_by_telephone(){
    createRequest();
    var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=set_fields_by_telephone&oID=<?php echo $_GET['oID']; ?>";
    xmlHttp.open("GET", url, true);
    xmlHttp.onreadystatechange = function(){
        if(xmlHttp.readyState!=4)return;
        if(xmlHttp.status==200){
            var info = jQuery.parseJSON(xmlHttp.responseText);
            if (info.match_located=='1'){
                $('input[name="update_customer_name"]').val(info.name);
                $('input[name="update_customer_company"]').val(info.company);
                $('input[name="update_customer_street_address"]').val(info.address1);
                $('input[name="update_customer_suburb"]').val(info.address2);
                $('input[name="update_customer_city"]').val(info.city);
                $('input[name="update_customer_state"]').val(info.state);
                $('input[name="update_customer_postcode"]').val(info.postcode);
                //$('input[name="update_customer_telephone"]').val(info.telephone);
                $('input[name="update_customer_email_address"]').val(info.email);
            }
        }
    };
...
function set_customer_details_by_email(){
    createRequest();
    var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=set_fields_by_email&oID=<?php echo $_GET['oID']; ?>";
    xmlHttp.open("GET", url, true);
    xmlHttp.onreadystatechange = function(){
        if(xmlHttp.readyState!=4)return;
        if(xmlHttp.status==200){
...
function updateProductsField(action, pid, field, value, info) {
      ...
      createRequest();
        ...
        var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=update_product_field&oID=<?php echo $_GET['oID']; ?>&pid=" + pid + "&field=" + field + "&new_value=" + value;
		...
		var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=update_product_value_field&oID=<?php echo $_GET['oID']; ?>&pid=" + pid + "&price=" + price + "&final_price=" + final_price;
		...
		if ( (action == 'delete') && (field == 'delete') && (value == true) ){
		  ...
		  if (confirm('<?php echo AJAX_CONFIRM_PRODUCT_DELETE; ?>')) {
           ...
           var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=delete_product_field&oID=<?php echo $_GET['oID']; ?>&pid=" + pid + "&field=" + field + "&new_value=" + value;
  ...
  function updateAttributesField (action, field, aid, pid, value) {
	 ...
	 createRequest();        
		...
		if (action == 'simple') {
		var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=update_attributes_field&oID=<?php echo $_GET['oID']; ?>&aid=" + aid +"&pid=" + pid + "&field=" + field + "&new_value=" + value;
		}
	 
        if (action == 'hard') {
	    var final_price = document.getElementById("update_products[" + pid + "][final_price]").value;
		var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=update_attributes_field&oID=<?php echo $_GET['oID']; ?>&aid=" + aid +"&pid=" + pid + "&field=" + field + "&new_value=" + value + "&final_price=" + final_price;
		}
	...
	function updateDownloads (field, did, pid, value) {
	  ...
	  createRequest();
	    ...
	    var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=update_downloads&oID=<?php echo $_GET['oID']; ?>&pid=" + pid + "&field=" + field + "&new_value=" + value + "&did=" + did;
...
function updateCommentsField(action, id, status, value, info) {
      ...
      if ( (status) && (status == true) && (action == 'delete') ) {
	     if (confirm('<?php echo AJAX_CONFIRM_COMMENT_DELETE; ?>')) {
			...
			createRequest();
		    var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=delete_comment&cID=" + id + "&oID=<?php echo $_GET['oID']; ?>";
		    xmlHttp.open("GET", url, true);
	 ...
	 if (action == 'update') {
	  		...
	  		createRequest();
		    var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=update_comment&cID=" + id + "&comment=" + value + "&oID=<?php echo $_GET['oID']; ?>";
		    xmlHttp.open("GET", url, true);
...
function updateShippingZone(field, value) {
  ...
  function setShipping(method) {
	...
	<?php if (ORDER_EDITOR_USE_AJAX == 'true') { ?>obtainTotals();<?php } ?>
		    ...
		    var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=insert_shipping&title=" + urlencode(title) + "&id=" + id + "&value=" + value + "&sort_order=" + sort_order + "&oID=<?php echo $_GET['oID']; ?>";
		    xmlHttp.open("GET", url, true);
  ...
  echo '                  }' . "\n";
		...
		createRequest();
	    var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=update_currency&oID=<?php echo $_GET['oID']; ?>&table=<?php echo TABLE_ORDERS; ?>&currency=" + currency + "&currency_value=" + currency_value;
  ...
  function clearComments() {
   ...
   function getNewComment() { //this is used for inserting new commments
	   ...
	   var usps_track_num = encodeURIComponent(document.getElementById("usps_track_num").value);
	   var usps_track_num2 = encodeURIComponent(document.getElementById("usps_track_num2").value);
	   var ups_track_num = encodeURIComponent(document.getElementById("ups_track_num").value);
	   var ups_track_num2 = encodeURIComponent(document.getElementById("ups_track_num2").value);
	   var fedex_track_num = encodeURIComponent(document.getElementById("fedex_track_num").value);
	   var fedex_track_num2 = encodeURIComponent(document.getElementById("fedex_track_num2").value);
	   var dhl_track_num = encodeURIComponent(document.getElementById("dhl_track_num").value);
	   var dhl_track_num2 = encodeURIComponent(document.getElementById("dhl_track_num2").value);*/
	   var notifyComments = '';
	   var comments = '';
	   var usps_track_num = '';
	   var usps_track_num2 = '';
	   ...
	   var ups_track_num2 = '';
	   var fedex_track_num = '';
	   var fedex_track_num2 = '';
	   var dhl_track_num = '';
	   var dhl_track_num2 = '';
	   var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=insert_new_comment&oID=<?php echo $_GET['oID']; ?>&status=" + status +"&notify=" + notify + "&notify_comments=" + notifyComments + "&comments=" + comments+ "&usps_track_num=" + usps_track_num+ "&usps_track_num2=" + usps_track_num2+ "&ups_track_num=" + ups_track_num+ "&ups_track_num2=" + ups_track_num2+ "&fedex_track_num=" + fedex_track_num+ "&fedex_track_num2=" + fedex_track_num2+ "&dhl_track_num=" + dhl_track_num+ "&dhl_track_num2=" + dhl_track_num2;
  ...
  function obtainTotals() { //this is used for processing/updating order totals
   ...
   function newOrderEmail() { //sending out new order confirmation emails the AJAX way
     ...
     if (confirm('<?php echo AJAX_NEW_ORDER_EMAIL; ?>')) {
	    ...
	    createRequest();
	    //if (comments) { 
		//var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=new_order_email&comments=" + encodeURIComponent(comments) + "&oID=<?php echo $_GET['oID']; ?>";
		//} else {
		var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX_POS; ?>?action=new_order_email&oID=<?php echo $_GET['oID']; ?>";
		//}
...
function init()
{
  ...
  if (optional) {
  if (paymentMethod == '<?php echo preg_replace("/[ ]+/", " ", ORDER_EDITOR_CREDIT_CARD); ?>') {
...
function newwindowFlatRate() 
{ 
window.open('shippingdetails_flatrate.html','jav','width=300,height=500,resizable=yes'); 
} 
function newwindowStandard() 
{ 
window.open('shippingdetails_standard.html','jav','width=300,height=550,resizable=yes'); 
}
