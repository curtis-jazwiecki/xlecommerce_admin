...
?>
              ...
              <tr>
                <td colspan="2" align="right"><?php echo tep_image_submit('button_send_mail_b.gif', IMAGE_SEND_EMAIL); ?></td>
              </tr>
            </table></td>
          </form></tr>
...
<?php      
  ...
  case 'voucheredit':
    ...
    $coupon=tep_db_fetch_array($coupon_query);
    $coupon_amount = (($coupon['coupon_amount'] == round($coupon['coupon_amount'])) ? number_format((float)$coupon['coupon_amount'], 2) : number_format((float)$coupon['coupon_amount'], 2));
    if ($coupon['coupon_type']=='P') {
      // not floating point value, don't display decimal info
      $coupon_amount = (($coupon_amount == round($coupon_amount)) ? number_format((float)$coupon_amount) : number_format((float)$coupon_amount, 2)) . '%';
    }
    if ($coupon['coupon_type']=='S') {
      $coupon_free_ship .= true;
    }
    $coupon_min_order = (($coupon['coupon_minimum_order'] == round($coupon['coupon_minimum_order'])) ? number_format((float)$coupon['coupon_minimum_order'], 2) : number_format((float)$coupon['coupon_minimum_order'], 2));
...
?>
      <tr>
        <td align="left"><?php echo tep_image_submit('button_preview_b.gif',IMAGE_PREVIEW); ?></td>
        <td align="left"><?php echo '&nbsp;&nbsp;<a href="' . tep_href_link('coupon_admin.php', ''); ?>"><?php echo tep_image_button('button_cancel_b.gif', IMAGE_CANCEL); ?></a>
      </td>
      </tr>
      </td></table></form>
      </tr>
...
<?php  
      if ($cc_list['coupon_type'] == 'P') {
        // not floating point value, don't display decimal info
        echo (($cc_list['coupon_amount'] == round($cc_list['coupon_amount'])) ? number_format((float)$cc_list['coupon_amount']) : number_format((float)$cc_list['coupon_amount'], 2)) . '%';
...
<?php
    ...
    default:
      ...
      if ($cInfo->coupon_type == 'P') {
        // not floating point value, don't display decimal info
        $amount = (($amount == round($amount)) ? number_format((float)$amount) : number_format((float)$amount, 2)) . '%';
      } else {
