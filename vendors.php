...
*/
  ...
  if (tep_not_null($action)) {
    ...
    case 'confirm':
        if (isset($_GET['vendors_id']) && $_GET['vendors_id']=='1'){
            tep_redirect(tep_href_link(FILENAME_VENDORS));
        }
        break;
      ...
      case 'deleteconfirm' :
        ...
        tep_db_query("update products set vendors_id='1' where vendors_id='" . (int) $vendors_id . "'");
        $vendor_deleted = true;
...
<?php
    if ($action == 'new') {
      ...
      echo tep_image_submit('button_update.gif', IMAGE_UPDATE, 'action=update') . ' <a href="' . tep_href_link(FILENAME_VENDORS, tep_get_all_get_params(array('action'))) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>';
    }        
?>
        </td>
      </tr>
...
<?php
    ...
    switch ($action) {
      ...
      case 'confirm' :
        ...
        $contents[] = array ( 'text' => '<b>CAUTION!! ' . $num_products . ' products are assigned to this vendor! <br><br></b>' . TEXT_DELETE_INTRO . '<br><br>');
