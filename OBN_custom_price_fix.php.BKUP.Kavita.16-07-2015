...
// Query to get the selected products and make the changes
	...
	echo 'SELECT p.products_id AS id, p.base_price AS price, p.lock_price, p.markup FROM products p, products_to_categories pcat WHERE p.products_id = pcat.products_id ' . $where_string;
    ...
    while ($products_update = tep_db_fetch_array($products_update_query)){
		if ($fixed == 0){  
			...
			$pos = stripos($products_update['markup'], 'margin');
			if ($pos!==false){
		...
		if($above_below == '1' && $above_below_2 == '2')
		  {
			tep_db_query("UPDATE " . TABLE_PRODUCTS . " 
				SET products_price='" . $new_price . "', markup='".$markup."', roundoff_flag = '$roundoff'
				WHERE products_id='" . $products_update['id'] . "' AND (base_price <= " . $above_below_value . " AND base_price > " . $above_below_value_2 . ") AND lock_price = '0' AND manual_price < '0.01'");
		  }
		elseif($above_below == '2' && $above_below_2 == '1')
		  {
			tep_db_query("UPDATE " . TABLE_PRODUCTS . " 
		...
		elseif($above_below == '1')
		  {
			  tep_db_query("UPDATE " . TABLE_PRODUCTS . " 
							SET products_price='" . $new_price . "', markup='".$markup."', roundoff_flag = '$roundoff'
            	        	WHERE products_id='" . $products_update['id'] . "' AND base_price <= '" . $above_below_value . "' AND lock_price = '0' AND manual_price < '0.01'");
		  }
		elseif($above_below == '2')
		  {
			  tep_db_query("UPDATE " . TABLE_PRODUCTS . " 
     	...
     	elseif ($products_update['lock_price'])
		  {
		  	  tep_db_query("UPDATE " . TABLE_PRODUCTS . " 
		    	            SET markup='".$markup."', roundoff_flag = '$roundoff'
            	    	    WHERE products_id='" . $products_update['id'] . "' AND lock_price = '0' AND manual_price < '0.01'");
		  }
		else
		  {
			  tep_db_query("UPDATE " . TABLE_PRODUCTS . " 
...
// Finish the rest of the update text information
	...
	echo '<br />' . "\n" . $price_updates_id . ' - ' . $update_string . "\n";
