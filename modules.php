  ...
  if (tep_not_null($set)) {
    switch ($set) {
      ...
      case 'ordertotal':
        ...
        break;
//BOF:amazon_interation
      case 'amazon':
            tep_redirect(tep_href_link(FILENAME_CONFIGURATION, 'gID=78'));
            break;
//EOF:amazon_interation
//BOF:amazon_interation
      case 'ebay':
            tep_redirect(tep_href_link(FILENAME_CONFIGURATION, 'gID=79'));
            break;
//EOF:amazon_interation
      case 'googlefeeder':
            tep_redirect(tep_href_link(FILENAME_CONFIGURATION, 'gID=81'));
            break;
      case 'payment':
      default:
        $module_type = 'payment';
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'save':
        ...
        if (!empty($vendor)){
            tep_redirect(tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $HTTP_GET_VARS['module'] . '&vendor=' . $vendor['id']));
        } else {
      ...
      case 'remove':
        ...
        if (!empty($vendor)){
            tep_redirect(tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $class . '&vendor=' . $vendor['id']));
        } else {
...
?>
                ...
                <td align="right"><?php if (isset($mInfo) && is_object($mInfo) && ($class == $mInfo->code) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $class ) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
  }
...
?>
              <tr>
                <td colspan="3"><?php echo TEXT_MODULE_DIRECTORY . ' ' . $module_directory; ?></td>
              </tr>
              <?php if (!empty($vendor)){ ?>
              <tr>
                <td>
                    <input type="button"  value="Back to Custom Shipping Manager" onClick="javascript:location.href='<?php echo tep_href_link('custom_shipping_modules.php'); ?>'" />
                </td>
              </tr>
              <?php } ?>
            </table></td>
