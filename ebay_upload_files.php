...
function getFeedXml($job_type, &$items){
	...
	if ($job_type=='AddFixedPriceItem' || $job_type=='ReviseFixedPriceItem' || $job_type=='RelistFixedPriceItem'){
		if ($job_type=='AddFixedPriceItem'){
			...
			$sql = tep_db_query("select * from products p inner join products_description pd on (p.products_id=pd.products_id and pd.language_id='1') inner join products_extended pe on p.products_id=pe.osc_products_id left join ebay_product_feed_errors pfe on (p.products_model=pfe.sku and pfe.environment='" . EBAY_ENVIRONMENT . "') where (pfe.sku is null or pfe.ack!='failure') and p.products_status='1' and p.is_ebay_ok='1' and p.ebay_category_id>0 and item_listed_on_ebay='0' and p.products_quantity>='" . (int)EBAY_MIN_STOCK_QTY . "'");
		...
		if ($sql){
			if (tep_db_num_rows($sql)){
				...
				while($entry = tep_db_fetch_array($sql)){
                    ...
                    $xml .=             "<ReturnPolicy>\n";
					$xml .=                 "<Description><![CDATA[" . $return_policy . "]]></Description>\n";
                    $xml .=                 "<RefundOption>" . EBAY_RETURN_POLICY . "</RefundOption>\n";
                    $xml .=                 "<ReturnsAcceptedOption>" . EBAY_RETURN_POLICY_STATUS . "</ReturnsAcceptedOption>\n";
