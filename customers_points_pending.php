          ...
          <tr>
            ...
            <td align="right"><table class="table table-bordered table-hover">
              ...
              <tr><?php echo tep_draw_form('status', FILENAME_CUSTOMERS_POINTS_PENDING, '', 'get'); ?>
                <td align="right"><?php echo TABLE_HEADING_ORDERS_STATUS . ': ' .tep_draw_pull_down_menu('status', array_merge(array(array('id' => '', 'text' => TEXT_SHOW_ALL)), $orders_statuses)). '&nbsp;'. TABLE_HEADING_POINTS_STATUS . ':'. tep_draw_pull_down_menu('filter', $filter_array, '', 'onChange="this.form.submit();"'); ?></td>
              </form></tr>
            </table></td>
          </tr>
        </table></td>
...
?>
                <td><?php echo '<a href="' . tep_href_link(FILENAME_ORDERS . '?oID=' . $uInfo->orders_id . '&action=edit') . '">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW_EDIT) . '</a>&nbsp;' . $pending_points['customers_name']; ?></td>
                <td align="right"><?php echo strip_tags($pending_points['order_total']); ?></td>
                <td align="right"><?php echo str_replace('-', '', number_format($pending_points['points_pending'],POINTS_DECIMAL_PLACES)); ?></td>
                <td align="right"><?php echo str_replace('-', '', $currencies->format($pending_points['points_pending'] * REDEEM_POINT_VALUE)); ?></td>
                <td align="center"><?php echo $pending_points['orders_status_name']; ?></td>
                <td align="center"><?php echo $points_status_name; ?></td>
...
?>
              <tr>
                <td colspan="7"><table class="table table-bordered table-hover">
                  <tr>
                    <td><?php echo $pending_points_split->display_count($pending_points_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_ORDERS); ?></td>
