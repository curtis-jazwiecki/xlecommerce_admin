...
<?php
    ...
    $options = "select * from " . TABLE_PRODUCTS_OPTIONS . " where language_id = '" . (int)$languages_id . "' and is_xml_feed_option='0' order by " . $option_order_by;
...
<?php
    $per_page = MAX_ROW_LISTS_OPTIONS;
    $values = "select pov.products_options_values_id, pov.products_options_values_name, pov2po.products_options_id from products_options popt, " . TABLE_PRODUCTS_OPTIONS_VALUES . " pov left join " . TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS_OPTIONS . " pov2po on pov.products_options_values_id = pov2po.products_options_values_id where pov.language_id = '" . (int)$languages_id . "' and popt.is_xml_feed_option = '0' and popt.language_id = '" . (int)$languages_id . "' and pov2po.products_options_id = popt.products_options_id order by pov.products_options_values_id";
    if (!isset($value_page)) {
      $value_page = 1;
    }
...
<?php
        $options = tep_db_query("select products_options_id, products_options_name from " . TABLE_PRODUCTS_OPTIONS . " where language_id = '" . (int)$languages_id . "' and is_xml_feed_option='0' order by products_options_name");
        while ($options_values = tep_db_fetch_array($options)) {
...
<?php
      $options = tep_db_query("select products_options_id, products_options_name from " . TABLE_PRODUCTS_OPTIONS . " where language_id = '" . $languages_id . "' and popt.is_xml_feed_option = '0' order by products_options_name");
      while ($options_values = tep_db_fetch_array($options)) {
