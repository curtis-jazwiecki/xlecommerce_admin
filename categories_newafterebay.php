      ...
      case 'update_product':
        if (isset($HTTP_POST_VARS['edit_x']) || isset($HTTP_POST_VARS['edit_y'])) {
          ...
          $sql_data_array = array('products_quantity' => tep_db_prepare_input($HTTP_POST_VARS['products_quantity']),
                                  ...
                                  'products_size' => tep_db_prepare_input($HTTP_POST_VARS['prodcust_size']),
                                  /********************PRODUCT SIZE MOD BY FIW**************************/
...
<?php
  if ($action == 'new_product') {
    ...
    if (isset($HTTP_GET_VARS['pID']) && empty($HTTP_POST_VARS)) {
    	/********************PRODUCT SIZE MOD BY FIW**************************/
      $product_query = tep_db_query("select pd.products_name, pd.products_description, pd.products_head_title_tag, pd.products_head_desc_tag, pd.products_head_keywords_tag, pd.products_url, p.products_id, p.products_quantity, p.products_size, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, date_format(p.products_date_available, '%Y-%m-%d') as products_date_available, p.products_status, p.products_tax_class_id, p.manufacturers_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'");
      /********************PRODUCT SIZE MOD BY FIW**************************/
      $product = tep_db_fetch_array($product_query);
    ...
    /********************PRODUCT SIZE MOD BY FIW**************************/
     if (!isset($pInfo->products_size)) $pInfo->products_size = '1';
    switch ($pInfo->products_size) {
      case '0': $in_size = false; $out_size = true; break;
      case '1':
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <? /********************PRODUCT SIZE MOD BY FIW**************************/ ?>
           <tr>
            <td class="main">Products Size:</td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_radio_field('products_size', '1', $in_size) . '&nbsp;Standard&nbsp;' . tep_draw_radio_field('products_size', '0', $out_size) . '&nbsp;Oversized'; ?></td>
          </tr>
...
<?php
    ...
    if (tep_not_null($HTTP_POST_VARS)) {
      ...
      $products_url = $HTTP_POST_VARS['products_url'];
    	...
    	/********************PRODUCT SIZE MOD BY FIW**************************/
      $product_query = tep_db_query("select p.products_id, pd.language_id, pd.products_name, pd.products_description,pd.products_head_title_tag, pd.products_head_desc_tag, pd.products_head_keywords_tag, pd.products_url, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.products_size, p.manufacturers_id  from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "'");
      /********************PRODUCT SIZE MOD BY FIW**************************/
      $product = tep_db_fetch_array($product_query);
    ...
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
      if (isset($HTTP_GET_VARS['read']) && ($HTTP_GET_VARS['read'] == 'only')) {
        ...
        $pInfo->products_head_keywords_tag = tep_db_prepare_input($products_head_keywords_tag[$languages[$i]['id']]);          $pInfo->products_url = tep_get_products_url($pInfo->products_id, $languages[$i]['id']);
      } else {
...
//Categry Status MOD BEGIN by FIW
      $categories_query = tep_db_query("select c.categories_id, cd.categories_name, c.categories_image, c.parent_id, c.sort_order, c.categories_status, c.date_added, c.last_modified, cd.categories_htc_title_tag, cd.categories_htc_desc_tag, cd.categories_htc_keywords_tag, cd.categories_htc_description from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' and cd.categories_name like '%" . tep_db_input($search) . "%' order by c.sort_order, cd.categories_name");
    } else {
...
?>
      ...
      if ($categories['categories_status'] == '1') {
        echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_CATEGORIES, 'action=setflagc&flag=0&cPath=' . $cPath) . '&cID=' . $categories['categories_id'] . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link(FILENAME_CATEGORIES, 'action=setflagc&flag=1&cPath=' . $cPath) . '&cID=' . $categories['categories_id'] . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
      }
...
<?php
    ...
    switch ($action) {
      case 'new_category':
        ...
        //Categroies Status MOD BEGIN by FIW
        $contents[] = array('text' => '<br>Categories Status<br>' . tep_draw_radio_field('categories_status', '1', '') . '&nbsp;Active&nbsp;' . tep_draw_radio_field('categories_status', '0', '') . '&nbsp;Inactive');
        //Categroies Status MOD END by FIW
...
/* Ebay modification here  by FIW */
            $contents[] = array('align' => 'center', 'text' => '<a href="fl_ebay.php?pID=' . $pInfo->products_id . '">In ebay</a><a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=new_product') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=delete_product') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a> <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=move_product') . '">' . tep_image_button('button_move.gif', IMAGE_MOVE) . '</a> <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=copy_to') . '">' . tep_image_button('button_copy_to.gif', IMAGE_COPY_TO) . '</a>');
/* Ebay modification end  by FIW */            
