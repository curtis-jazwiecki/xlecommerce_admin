    ...
    <section>
        ...
        <section class="main-content">
            ...
            <div class="panel panel-default">
                <div class="panel-heading"><?php echo HEADING_TITLE; ?> <a href="#" data-perform="panel-dismiss" data-toggle="tooltip" title="Close Panel" class="pull-right"> <em class="fa fa-times"></em> </a> <a href="#" data-perform="panel-collapse" data-toggle="tooltip" title="Collapse Panel" class="pull-right"> <em class="fa fa-minus"></em> </a> </div>
...
if (($action == 'edit') && ($order_exists == true)) {
    ...
    ?>
                            <tr>
                                <td><table class="table table-bordered table-hover">
                                        <tr>
                                            <td colspan="2"><?php echo HEADING_TITLE . (($order->customer['id'] == 0) ? ' <b>no account!</b>' : ''); ?></td>
                                        </tr>
                                        <tr>
                            ...
                            <tr>
                                <td><table class="table table-bordered table-hover">
                                        ...
                                        <tr>
                                            <td colspan="3"><b><u>IP Address: <?php echo $order->info['ip_address']; ?></u></b><span style="float:right;">

    <?php
    $is_returned_order = tep_db_fetch_array(tep_db_query("select parent_orders_id from orders where orders_id = '" . (int) $_GET['oID'] . "'"));

    if ($is_returned_order['parent_orders_id'] > '0') {

        echo '<a href="' . tep_href_link(FILENAME_ORDERS, 'selected_box=customers&oID=' . $is_returned_order['parent_orders_id'] . '&action=edit') . '">' . tep_image_button('button_parent_order.gif', IMAGE_PARENT_ORDERS) . '</a>';
    } else {
        $check_if_already_returned = tep_db_fetch_array(tep_db_query("select orders_id from orders where parent_orders_id = '" . (int) $_GET['oID'] . "'"));
        if ($check_if_already_returned['orders_id'] > 0) {

            echo '<a href="' . tep_href_link(FILENAME_ORDERS, 'selected_box=customers&oID=' . $check_if_already_returned['orders_id'] . '&action=edit') . '">' . tep_image_button('button_order_returned.gif', IMAGE_RETURNED_ORDERS) . '</a>';
        } else {

            echo '<a href="' . tep_href_link(FILENAME_RETURN_ORDERS, 'oID=' . $_GET['oID']) . '">' . tep_image_button('button_return_order.gif', IMAGE_RETURN_ORDERS) . '</a>';
        }
    }
    ?></span></td>
                                        </tr>
                                        <tr>
                                            <td colspan="3"> Order Placed By:
    <?php
                                        ...
                                        <tr>
                                            <td><table class="table table-bordered table-hover">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><?php echo '<a href="mailto:' . $order->customer['email_address'] . '" style="color: #333"><u>' . $order->customer['email_address'] . '</u></a>'; ?></td>
                                                    </tr>
                                                </table></td>
                                            <td><table class="table table-bordered table-hover">
                                                    <tr>
    ...
    if ($order->info['is_capture'] == '0') {
        ?>
                                            <tr>
                                                <td><b><?php echo 'To capture payment click here:'; ?></b></td>
                                                <td><?php
    ...
    for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
        ...
        $package_product_color = '';
        if ($order->products[$i]['package_product_id'] == '1') {

            $package_product_color = 'red';
        }



        echo '          <tr class="dataTableRow">' . "\n" . '            <td class="dataTableContent" valign="top"><span style="color:' . $package_product_color . '">' . $order->products[$i]['name'] . '</span>';
        ...
        $send_to_obn_string = '<input type="checkbox" name="chk_send_to_obn[]" id="chk_send_to_obn[]" value="' . $order->products[$i]['orders_products_id'] . '" ' . (empty($order->products[$i]['sent_to_obn']) ? '' : ' disabled ' ) . ' />';
        
        if(tep_db_num_rows(tep_db_query("select 1 from products_bundles where bundle_id = (select products_id from orders_products where orders_products_id = '".$order->products[$i]['orders_products_id']."')"))){
            $send_to_obn_string = '';
        }
    ...
    ?>
                            ...
                            <tr>
                                <td align="right" colspan="10"><table class="table table-bordered table-hover">
    <?php
    ...
    if (is_multiple_vendor_order($_GET['oID'])) {
        ...
        foreach ($vendors as $vendor) {
            ...
            if (tep_db_num_rows($query_ups_shipping_query)) {
                ...
                echo '<b>' . $vendor['name'] . '--' . TABLE_HEADING_UPS_TRACKING . ': </b>' . tep_draw_textbox_field('ups_track_num_' . $vendor['id'], '40', '40', '', $vendor['ups_tracking']);
                ?>
            ...
            if (tep_db_num_rows($query_usps_shipping_query)) {
                ...
                echo '<b>' . $vendor['name'] . '--' . TABLE_HEADING_USPS_TRACKING . ': </b>' . tep_draw_textbox_field('usps_track_num_' . $vendor['id'], '40', '40', '', $vendor['usps_tracking']);
                ?>
                                                        &nbsp; <a target="_blank" href="http://trkcnfrm1.smi.usps.com/PTSInternetWeb/InterLabelInquiry.do?origTrackNum=<?php echo $vendor['usps_tracking']; ?>"><?php echo tep_image_button('button_track.gif', 'Track Package'); ?></a>
            ...
            <?php } ?>
                                                    ...
                                                    <!-- fedex tracking details starts --></td>
                                            </tr>
            <?php
        }
        ?>
                                        ...
                                        <tr>
                                            <td><table class="table table-bordered table-hover">
                                                    <tr>
                                                        ...
                                                        <td><a href="display_usps_label.php?oID=<?php echo $_GET['oID']; ?>" target="_blank" id="uspsGetLabel" style="color:black;">Get Label</a></td>
                                                    </tr>
                                                    <tr>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><a href="display_ups_label.php?oID=<?php echo $_GET['oID']; ?>" target="_blank" id="fedexGetLabel" style="color:black;">Get Label</a></td>
                                                    </tr>
                                                    <tr>
                                                        <td colspan="3"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                                                    </tr>
                                                    <tr>
                                                        ...
                                                        <td><a href="fedex_popup_thermal.php?oID=<?php echo $_GET['oID']; ?>" target="_blank" id="fedexGetLabel" style="color:black;">Get Label</a></td>
                                                    </tr>
                                                    <tr>
                                                        <td colspan="3"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                                                    </tr>
    ...
    if (count($tracking_data) > 0) {
        ...
        <?php foreach ($tracking_data as $tracking) { ?>
                                                        <tr>
                                                            ...
                                                            <td><a target="_blank" href="<?php echo $tracking['link']; ?>"> <img src="<?= DIR_WS_LANGUAGES . $language . '/images/buttons/'; ?>button_track.gif" alt="Track Packages" border="0"> </a></td>
                                                        </tr>
            <?php }
        ?>
                                                </table></td>
    ...
    <?php
    // unset all the other status if this is a returned order #start
    if ($order->info['orders_status'] == '12') {
        foreach ($orders_statuses as $okey => $ostatus) {
            if ($ostatus['id'] != '12') {
                unset($orders_statuses[$okey]);
            }
        }
    } else {
        foreach ($orders_statuses as $okey => $ostatus) {
    ...
    // unset all the other status if this is a returned order #ends
    ...
    echo '<a href="' . tep_href_link(FILENAME_ORDERS_EDIT, 'oID=' . $_GET['oID']) . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a>' .
    ...
    while ($orders = tep_db_fetch_array($orders_query)) {
        ...
        ?></td>
                                                                ...
                                                                <td align="right"><?php if (isset($oInfo) && is_object($oInfo) && ($orders['orders_id'] == $oInfo->orders_id)) {
