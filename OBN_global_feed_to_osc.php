...
define('OBN_FEED_SOS_URL',
    'http://67.227.172.78/admin/send_reqd_info_to_retailer.php');
...
");
        ...
        while ($row = tep_db_fetch_array($price_update_query)) {
            ...
            if ($above_below == '0') {
                ...
                $where_string .= " AND (p.base_price <= " . $above_below_value .
                    " AND p.base_price > " . $above_below_value_2 . ")";
            ...
            $products_update_query = tep_db_query('SELECT p.products_id AS id, p.base_price AS price, p.lock_price, p.markup FROM products p, products_to_categories pcat WHERE p.products_id = pcat.products_id ' .
                $where_string);
            ...
            while ($data = tep_db_fetch_array($products_update_query)) {
                ...
                tep_db_query("insert into temp_markup_price set
                    ...
                    `roundoff` = '" . $roundoff .
                    "' on duplicate key update markup='" . $markup . "',price_update_add='" . $add .
                    "',price_update_fixed='" . $fixed . "',price_update_value='" . $value .
                    "', roundoff='" . $roundoff . "'");
    ...
    private function tep_get_child_categories($parent_id = '0', $category_tree_array = '')
    {
        ...
        while ($categories = tep_db_fetch_array($categories_query)) {
            ...
            $category_tree_array = $this->tep_get_child_categories($categories['categories_id'],
                $category_tree_array);
  ...
  `products_last_modified` datetime DEFAULT NULL,  KEY `products_model` (`products_model`)) ENGINE=MyISAM DEFAULT CHARSET=latin1");
        ...
        if (empty($this->categories)) {
            ...
            if (file_exists($dir . 'OBN_categories.xml')) {
                ...
                foreach ($categories_xml->children() as $category) {
                    ...
                    $this->add_category_to_osc($temp_id, $temp_name, ($temp_parent_id ? $temp_parent_id :
                        '0'));
        ...
        if (empty($this->product_options)) {
            ...
            if (file_exists($dir . 'OBN_options.xml')) {
                ...
                foreach ($options_xml->ProductOptions->children() as $product_option) {
                    ...
                    $temp_name = htmlspecialchars_decode((string )$product_option->{
                        NODE_PRODUCT_OPTION_NAME});
                ...
                foreach ($options_xml->ProductOptionValues->children() as $product_option_value) {
                    ...
                    $temp_name = htmlspecialchars_decode((string )$product_option_value->{
                        NODE_PRODUCT_OPTION_VALUE_NAME});
            ...
            */
            ...
            foreach ($this->xml->{NODE_PRODUCTS}->children() as $product) {
                ...
                if ($products_status) {
					...
					$sql = tep_db_query("select p.products_id,p.lock_status,p.products_price,p.base_price,p.markup,p.lock_price,p.roundoff_flag,tmp.products_id as mpt_products_id,tmp.markup as mpt_markup,tmp.roundoff as mpt_roundoff,tmp.price_update_add,tmp.price_update_fixed,tmp.price_update_value,IF(sp.specials_new_products_price, sp.discount, NULL) as discount from products as p left join temp_markup_price as tmp USING(products_id) left join specials as sp USING(products_id) where products_model='" .str_replace("'", "''", $temp_prod_model) . "'");
                    ...
                    if ($parent_products_model_exists && !empty($temp_parent_prod_model) && $temp_parent_prod_model !=
                        'NULL') {
                    ...
                    if (!empty($temp_prod_map) && $temp_prod_map > 0) {
                        ...
                        if ($flag_manual_price || (OBN_POPULATE_MANUAL_PRICE == 'true' &&
                            OBN_ACTIVATE_MANUAL_PRICE_FOR_ALL_ITEMS == 'true')) {
                    ...
                    if ($prod_exists) {
                        ...
                        tep_db_perform('products_description', $sql_data_array, 'update',
                            "products_id = '" . $temp_prod_osc_id . "' and language_id='1'");
                    ...
                    */
                    ...
                    if ($prod_exists) {
                        ...
                        tep_db_perform('products_extended', $sql_data_array, 'update',
                            "osc_products_id = '" . $temp_prod_osc_id . "'");
                    ...
                    foreach ($product->{NODE_PRODUCT_ATTRIBUTES}->children() as $attribute) {
                        ...
                        if (tep_db_num_rows($sql)) {
                            ...
                            tep_db_perform('products_attributes', $sql_data_array, 'update',
                                "products_attributes_id = '" . $sql_info['products_attributes_id'] . "'");
    ...
    private function add_product_option_to_osc($option_feed_id, $option_name)
    {
        ...
        if (!empty($option_name)) {
            ...
            $sql = tep_db_query("select products_options_id
								...
								from products_options where products_options_name='" . str_replace("'",
                "''", $option_name) . "'");
    ...
    protected function get_price_with_markup($base_price, $markup)
    {
        ...
        if (empty($markup)) {
            ...
            return ($markup_in_negative ? ($base_price - ($markup_in_percent ? (($base_price *
                $markup_figure) / 100) : $markup_figure)) : ($base_price + ($markup_in_percent ?
                (($base_price * $markup_figure) / 100) : $markup_figure)));
