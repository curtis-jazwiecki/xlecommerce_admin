...
if (isset($_GET['action']) && $_GET['action'] == 'getSelectedProduct')
{
    ...
    if (is_array($pid_array) && !empty($pid_array) && !empty($_GET['pid']))
    {
        ...
        {
            $prodListStr .= '<tr id="child-' . $productList['products_id'] .
                '" class="dataTableRow" ><td  class="dataTableContent">' . $productList['products_name'] .
                '</td></tr>';
        }
...
//MVS end
                ...
                if ($HTTP_POST_VARS['products_bundle'] == "yes")
                {
                    ...
                    { // calculate total MSRP and weight from subproducts
                        $msrp = 0;
                        $weight = 0;
                    ...
                    'insert_product')
                {
                    for ($i = 0, $n = sizeof($languages); $i < $n; $i++)
                    {
                        ...
                        foreach ($products_id_arr as $val)
                        {
                            ...
                            {
                                tep_db_perform(TABLE_PRODUCTS_DESCRIPTION, $sql_data_array, 'update',
                ...
                } else
                {
                    ...
                    for ($i = 0, $n = sizeof($languages); $i < $n; $i++)
                    {
                        ...
                        //BOF:hash task
                        $registered_specifications = $_POST['registered_specification'];
                        if (!empty($registered_specifications)){
                            tep_db_query("delete from products_attributes where products_id='" . (int)$products_id . "' and options_id not in (" . implode(',', $registered_specifications) . ")");
                        }
                        ...
                        if (!empty($specifications)){
                            foreach($specifications as $specification){
                                ...
                                if (!empty($specification)){
                                    $options_query = tep_db_query("select products_options_id from products_options where language_id='1' and products_options_name='" . tep_db_prepare_input($specification) . "'");
                                    if (!tep_db_num_rows($options_query)){
                                        $option_id_query = tep_db_query("select max(products_options_id) as count from products_options");
                                        $info = tep_db_fetch_array($option_id_query);
                                        $products_options_id = $info['count']+1;
                                        $sql_data = array(
                                            'products_options_id' => $products_options_id, 
                                            'language_id' => '1', 
                                            'products_options_name' => $specification, 
                                        );
                                        tep_db_perform('products_options', $sql_data);
                                    } else {
                                        $info = tep_db_fetch_array($options_query);
                                        $products_options_id = $info['products_options_id'];
                                    }
                                    
                                    $attributes_query = tep_db_query("select products_attributes_id from products_attributes where products_id='" . (int)$products_id . "' and options_id='" . (int)$products_options_id . "'");
                                    if (!tep_db_num_rows($attributes_query)){
                                        $sql_data = array(
                                            'products_id' => $products_id, 
                                            'options_id' => $products_options_id, 
                                        );
                                        tep_db_perform('products_attributes', $sql_data);
                                    }
                                }
                            }
                        }
        ...
        */
        ...
        if (empty($pInfo->parent_products_model))
        {
            ...
            if (tep_db_num_rows($checkForChildQuery))
            {
                ...
                {
                    $child_products_array[$checkForChild['products_id']] = array('name' => $checkForChild['products_name'],
...
//MVS end
    ...
    $pInfo->qpu_price = ($roundoff_flag ? apply_roundoff(round($price, 4)) : round($price,
        4));
...
?></td>
    ...
    echo tep_draw_separator('pixel_trans.gif', HEADING_IMAGE_WIDTH,
...
?>
    ...
    echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .
        tep_draw_checkbox_field('is_fullday_price', '1', ($pInfo->is_fullday_price ? true : false),
        '', ($pInfo->is_lane_item ? '' : 'disabled="disabled"'));
...
?></td>
    ...
    echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .
        ...
        TEXT_PRODUCT_AVAILABLE . '&nbsp;' . tep_draw_radio_field('products_status', '0',
        $out_status) . '&nbsp;' . TEXT_PRODUCT_NOT_AVAILABLE;
...
?>&nbsp;</td>
        ...
        echo tep_draw_textarea_field('products_tags[' . $languages[$i]['id'] . ']',
            'soft', '70', '5', (isset($products_tags[$languages[$i]['id']]) ? stripslashes($products_tags[$languages[$i]['id']]) :
...
?></td>
    ...
    echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .
        tep_draw_file_field('products_image') . '<br>' . tep_draw_separator('pixel_trans.gif',
        '24', '15') . '&nbsp;' . $pInfo->products_image . tep_draw_hidden_field('products_previous_image',
...
?></td>
    ...
    echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .
        tep_draw_file_field('products_mediumimage') . '<br>' . tep_draw_separator('pixel_trans.gif',
        '24', '15') . '&nbsp;' . $pInfo->products_mediumimage . tep_draw_hidden_field('products_previous_mediumimage',
        $pInfo->products_mediumimage);
...
?></td>
    ...
    echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .
        tep_draw_file_field('products_largeimage') . '<br>' . tep_draw_separator('pixel_trans.gif',
        '24', '15') . '&nbsp;' . $pInfo->products_largeimage . tep_draw_hidden_field('products_previous_largeimage',
        $pInfo->products_largeimage);
...
?></td>
    ...
    echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .
        tep_draw_file_field('products_image_2') . '<br>' . tep_draw_separator('pixel_trans.gif',
        '24', '15') . '&nbsp;' . $pInfo->product_image_2 . tep_draw_hidden_field('products_previous_image_2',
        $pInfo->product_image_2);
...
?></td>
    ...
    echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .
        tep_draw_file_field('products_image_3') . '<br>' . tep_draw_separator('pixel_trans.gif',
        '24', '15') . '&nbsp;' . $pInfo->product_image_3 . tep_draw_hidden_field('products_previous_image_3',
        $pInfo->product_image_3);
...
?></td>
    ...
    echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .
        tep_draw_file_field('products_image_4') . '<br>' . tep_draw_separator('pixel_trans.gif',
        '24', '15') . '&nbsp;' . $pInfo->product_image_4 . tep_draw_hidden_field('products_previous_image_4',
        $pInfo->product_image_4);
...
?></td>
    ...
    echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .
        tep_draw_file_field('products_image_5') . '<br>' . tep_draw_separator('pixel_trans.gif',
        '24', '15') . '&nbsp;' . $pInfo->product_image_5 . tep_draw_hidden_field('products_previous_image_5',
        $pInfo->product_image_5);
...
?></td>
    ...
    echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .
        tep_draw_file_field('products_image_6') . '<br>' . tep_draw_separator('pixel_trans.gif',
        '24', '15') . '&nbsp;' . $pInfo->product_image_6 . tep_draw_hidden_field('products_previous_image_6',
        $pInfo->product_image_6);
...
?>
 &nbsp;</td>
       <td class="main"><?php
        ...
        {
            echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .
                tep_draw_input_field('sppcprice[' . $customers_group['customers_group_id'] . ']',
                $attributes['customers_group_price']);
        } else
        {
            if (isset($pInfo->sppcprice[$customers_group['customers_group_id']]))
            { // when a preview was done and the back button used
...
?>
    ...
    echo tep_draw_separator('pixel_trans.gif', '24', '15') . tep_draw_pull_down_menu('products_bundle',
        array(array('id' => 'no', 'text' => 'No'), array('id' => 'yes', 'text' => 'Yes')),
        $pInfo->products_bundle) .
...
?>
    ...
    echo tep_draw_hidden_field('bundled_subproducts_i', $i,
        'id="bundled_subproducts_i"');
    echo 'Select category : <br/>' . tep_draw_pull_down_menu('cPath',
        tep_get_category_tree(), '', 'onChange="get_products(this.value);"');
...
?>
    ...
    <tr>
        <td colspan="2" class="main"><b>Manage Specifications:</b></td>
    </tr>
    <tr>
        <td colspan="2">
            <table>
...
?>
                <tr>
                    <td colspan="2" class="main">Associated Specifications:</td>
                </tr>
<?php
    }
    foreach ($specifications as $specification){
?>
                <tr>
                    <td valign="top" class="main">
                        <input type="checkbox" name="registered_specification[]" value="<?php echo $specification['id']; ?>" checked />
                    </td>
                    <td valign="top" class="main">
                    <?php echo $specification['text']; ?>
                    </td>
                </tr>
<?php
    }
?>
                <tr>
                    <td colspan="2" class="main">New Specification(s):</td>
                </tr>
<?php
    $options = array();
    $options[] = array(
    ...
    $options_query = tep_db_query("select products_options_name as text from products_options order by products_options_name");
    ...
    ?>
                <tr>
                    <td valign="top" class="main">1:</td>
                    <td valign="top" class="main">
                    <?php echo tep_draw_pull_down_menu('specification[]', $options); ?>
                    </td>
                </tr>
                <tr>
                    <td valign="top" class="main">2:</td>
                    <td valign="top" class="main">
                    <?php echo tep_draw_pull_down_menu('specification[]', $options); ?>
                    </td>
                </tr>
                <tr>
                    <td valign="top" class="main">3:</td>
                    <td valign="top" class="main">
                        <input type="text" name="specification[]" />
                    </td>
                </tr>
                <tr>
                    <td valign="top" class="main">4:</td>
                    <td valign="top" class="main">
                        <input type="text" name="specification[]" />
                    </td>
                </tr>
                <tr>
                    <td valign="top" class="main">5:</td>
                    <td valign="top" class="main">
                        <input type="text" name="specification[]" />
                    </td>
                </tr>
...
?></td>
            <td class="main" id="prModelText"><?php

        echo $pInfo->parent_products_model . '<br>' . $get_parent_name['products_name'];
...
?></td>
        ...
        echo '<input onclick="popupWindow(\'' . tep_href_link('add_edit_parent_products.php',
            (!empty($_GET['pID']) ? 'pID=' . $_GET['pID'] . '&model=' . $pInfo->
...
?></td>
    ...
    } else
    {
        ...
        $keystr = '<input id="child_ids" type="hidden" value="' . rtrim($keystr, ',') .
            '" name="child_ids">';
...
?>
        ...
        echo '<input onclick="popupWindow(\'' . tep_href_link('add_edit_child_products.php',
            (!empty($_GET['pID']) ? 'pID=' . $_GET['pID'] . '&model=' . $pInfo->
...
?></td>
          ...
          <tr bgcolor="#DBEAF9">
            <td colspan="2" class="main"><div id="childprod"><?php

        echo $childProductStr;
...
?>
        ...
        foreach ($bdata as $bundle_data)
        {
            ...
            echo tep_image(DIR_WS_CATALOG_IMAGES . $bundle_data['products_image'], $bundle_data['products_name'],
                intval(SMALL_IMAGE_WIDTH / 2), intval(SMALL_IMAGE_HEIGHT / 2),
                'hspace="1" vspace="1"') . '</td>';
            // comment out the following line to hide the subproduct qty
            echo "<td class=main align=right><b>" . $bundle_data['subproduct_qty'] .
                "&nbsp;x&nbsp;</b></td>";
            echo '<td class=main><a href="' . tep_catalog_href_link('product_info.php',
                'products_id=' . (int)$bundle_data['products_id']) .
            ...
            echo '</td>';
            echo '<td align=right class=main><b>&nbsp;' . $currencies->display_price($bundle_data['products_price'],
                tep_get_tax_rate($pInfo->products_tax_class_id)) . "</b></td></tr>\n";
...
?>
    ...
    echo tep_draw_form($form_action, FILENAME_CATEGORIES, 'cPath=' . $cPath . (isset
        ($HTTP_GET_VARS['pID']) ? '&pID=' . $HTTP_GET_VARS['pID'] : '') . '&action=' . $form_action,
    ...
    for ($i = 0, $n = sizeof($languages); $i < $n; $i++)
    {
        if (isset($HTTP_GET_VARS['read']) && ($HTTP_GET_VARS['read'] == 'only'))
        {
            ...
            $pInfo->products_description = tep_get_products_description($pInfo->products_id,
                $languages[$i]['id']);
...
<?php
        ...
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++)
        {
            echo tep_draw_hidden_field('products_name[' . $languages[$i]['id'] . ']',
                htmlspecialchars(stripslashes($products_name[$languages[$i]['id']])));
            echo tep_draw_hidden_field('products_description[' . $languages[$i]['id'] . ']',
                htmlspecialchars(stripslashes($products_description[$languages[$i]['id']])));
            ...
            echo tep_draw_hidden_field('products_tags[' . $languages[$i]['id'] . ']',
                htmlspecialchars(stripslashes($products_tags[$languages[$i]['id']])));
            ...
            echo tep_draw_hidden_field('products_url[' . $languages[$i]['id'] . ']',
                htmlspecialchars(stripslashes($products_url[$languages[$i]['id']])));
        }
...
<?php
    ...
    echo '<span class="smallText2">' . HEADING_TITLE_GOTO . ' ' .
        tep_draw_pull_down_menu('cPath', tep_get_category_tree(), $current_category_id,
...
?>
        ...
        {
            echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN,
                10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_CATEGORIES,
                'action=setflagc&flag=0&cPath=' . $cPath) . '&cID=' . $categories['categories_id'] .
        ...
        } else
        {
            echo '<a href="' . tep_href_link(FILENAME_CATEGORIES,
                'action=setflagc&flag=1&cPath=' . $cPath) . '&cID=' . $categories['categories_id'] .
                '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif',
                IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES .
...
?>
        ...
        if ($categories['is_amazon_ok'] == '1')
        {
            echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN,
                10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_CATEGORIES,
                'action=setamazonflagc&amazonflag=0&cPath=' . $cPath) . '&cID=' . $categories['categories_id'] .
                '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif',
                IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
        } else
        {
            echo '<a href="' . tep_href_link(FILENAME_CATEGORIES,
                'action=setamazonflagc&amazonflag=1&cPath=' . $cPath) . '&cID=' . $categories['categories_id'] .
                '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif',
                IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES .
                'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
        }
...
?>
        ...
        if ($categories['is_ebay_ok'] == '1')
        {
            echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN,
                10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_CATEGORIES,
                'action=setebayflagc&ebayflag=0&cPath=' . $cPath) . '&cID=' . $categories['categories_id'] .
                '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif',
                IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
        } else
        {
            echo '<a href="' . tep_href_link(FILENAME_CATEGORIES,
                'action=setebayflagc&ebayflag=1&cPath=' . $cPath) . '&cID=' . $categories['categories_id'] .
                '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif',
                IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES .
                'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
        }
...
<?php
        ...
        if ($products['products_status'] == '1')
        {
            echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN,
                10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_CATEGORIES,
                'action=setflag&flag=0&pID=' . $products['products_id'] . '&cPath=' . $cPath) .
                '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif',
                IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
        } else
        {
            echo '<a href="' . tep_href_link(FILENAME_CATEGORIES,
                'action=setflag&flag=1&pID=' . $products['products_id'] . '&cPath=' . $cPath) .
                '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif',
                IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES .
                'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
        }
...
?>
        ...
        if ($products['is_amazon_ok'] == '1')
        {
            echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN,
                10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_CATEGORIES,
                'action=setamazonflag&amazonflag=0&pID=' . $products['products_id'] . '&cPath=' .
                $cPath) . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif',
                IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
        } else
        {
            echo '<a href="' . tep_href_link(FILENAME_CATEGORIES,
                'action=setamazonflag&amazonflag=1&pID=' . $products['products_id'] . '&cPath=' .
                $cPath) . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif',
                IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES .
                'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
        }
...
?>
        ...
        if ($products['is_ebay_ok'] == '1')
        {
            echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN,
                10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_CATEGORIES,
                'action=setebayflag&ebayflag=0&pID=' . $products['products_id'] . '&cPath=' . $cPath) .
                '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif',
                IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
        } else
        {
            echo '<a href="' . tep_href_link(FILENAME_CATEGORIES,
                'action=setebayflag&ebayflag=1&pID=' . $products['products_id'] . '&cPath=' . $cPath) .
                '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif',
                IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES .
                'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
        }
...
?>
    ...
    echo tep_draw_pull_down_menu('move_selection_to_category_id',
...
?></td>
    ...
    if (!isset($HTTP_GET_VARS['search']))
        echo '<a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath .
            '&action=new_category') . '">' . tep_image_button('button_new_category.gif',
            IMAGE_NEW_CATEGORY) . '</a>&nbsp;<a href="' . tep_href_link(FILENAME_CATEGORIES,
            'cPath=' . $cPath . '&action=new_product') . '">' . tep_image_button('button_new_product.gif',
            IMAGE_NEW_PRODUCT) . '</a>';
...
<?php
    ...
    switch ($action)
    {
        case 'new_category':
            ...
            $contents = array('form' => tep_draw_form('newcategory', FILENAME_CATEGORIES,
            ...
            $contents[] = array('text' => '<br>Is category Group<br>' . tep_draw_radio_field
                    ('is_category_group', '1', false) . '&nbsp;Yes&nbsp;' . tep_draw_radio_field('is_category_group',
                    '0', true) . '&nbsp;No');
            //EOF:category_group
            ...
            $contents[] = array('text' => '<br>Categories Status<br>' . tep_draw_radio_field
                    ('categories_status', '1', '') . '&nbsp;Active&nbsp;' . tep_draw_radio_field('categories_status',
                    '0', '') . '&nbsp;Inactive');
            //Categroies Status MOD END by FIW
            ...
            $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.gif',
                    IMAGE_SAVE) . ' <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath) .
                    '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
            break;
        case 'edit_category':
            ...
            $contents = array('form' => tep_draw_form('categories', FILENAME_CATEGORIES,
                    ...
                    'enctype="multipart/form-data"') . tep_draw_hidden_field('categories_id', $cInfo->
                    categories_id));
            ...
            for ($i = 0, $n = sizeof($languages); $i < $n; $i++)
            {
                ...
                $category_htc_desc_string .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] .
                    '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' .
                    tep_draw_input_field('categories_htc_desc_tag[' . $languages[$i]['id'] . ']',
                    tep_get_category_htc_desc($cInfo->categories_id, $languages[$i]['id']));
                $category_htc_keywords_string .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] .
                    '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' .
                    tep_draw_input_field('categories_htc_keywords_tag[' . $languages[$i]['id'] . ']',
                    tep_get_category_htc_keywords($cInfo->categories_id, $languages[$i]['id']));
                $category_htc_description_string .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES .
                    $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) .
                    '&nbsp;' . tep_draw_textarea_field('categories_htc_description[' . $languages[$i]['id'] .
            ...
            $contents[] = array('text' => '<br>Is category Group<br>' . tep_draw_radio_field
                    ...
                    'disabled')) . '&nbsp;Yes&nbsp;' . tep_draw_radio_field('is_category_group', '0',
                    $cat_status_unset) . '&nbsp;No');
            //EOF:category_group
            ...
            $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.gif',
                    IMAGE_SAVE) . ' <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath .
                    '&cID=' . $cInfo->categories_id) . '">' . tep_image_button('button_cancel.gif',
                    IMAGE_CANCEL) . '</a>');
            break;
        case 'delete_category':
            ...
            $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif',
                    IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath .
                    '&cID=' . $cInfo->categories_id) . '">' . tep_image_button('button_cancel.gif',
                    IMAGE_CANCEL) . '</a>');
            break;
        case 'move_category':
            ...
            $contents[] = array('text' => '<br>' . sprintf(TEXT_MOVE, $cInfo->
                    categories_name) . '<br>' . tep_draw_pull_down_menu('move_to_category_id',
                    tep_get_category_tree(), $current_category_id));
            $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_move.gif',
                    IMAGE_MOVE) . ' <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath .
                    '&cID=' . $cInfo->categories_id) . '">' . tep_image_button('button_cancel.gif',
                    IMAGE_CANCEL) . '</a>');
            break;
        case 'delete_product':
            ...
            $contents = array('form' => tep_draw_form('products', FILENAME_CATEGORIES,
            ...
            $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif',
                    IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath .
                    '&pID=' . $pInfo->products_id) . '">' . tep_image_button('button_cancel.gif',
                    IMAGE_CANCEL) . '</a>');
            break;
        case 'move_product':
        ...
        case 'copy_to':
            ...
            $contents = array('form' => tep_draw_form('copy_to', FILENAME_CATEGORIES,
            ...
            $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_copy.gif',
                    IMAGE_COPY) . ' <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath .
                    '&pID=' . $pInfo->products_id) . '">' . tep_image_button('button_cancel.gif',
                    IMAGE_CANCEL) . '</a>');
            break;
        default:
            if ($rows > 0)
            {
                ...
                { // category info box contents
                    ...
                    $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_CATEGORIES,
                            'cPath=' . $cPath . '&cID=' . $cInfo->categories_id . '&action=edit_category') .
                            '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' .
                            tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $cInfo->
                            categories_id . '&action=delete_category') . '">' . tep_image_button('button_delete.gif',
                            IMAGE_DELETE) . '</a> <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' .
                    ...
                    if (tep_not_null($cInfo->last_modified))
                        $contents[] = array('text' => TEXT_LAST_MODIFIED . ' ' . tep_date_short($cInfo->
                                last_modified));
                    $contents[] = array('text' => '<br>' . tep_info_image($cInfo->categories_image,
                            $cInfo->categories_name, HEADING_IMAGE_WIDTH, HEADING_IMAGE_HEIGHT) . '<br>' . $cInfo->
                            categories_image);
...
// MVS end
                    $heading[] = array('text' => '<b>' . tep_get_products_name($pInfo->products_id,
                            $languages_id) . '</b>');
                    /* Ebay modification here !!!! */
                    $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_CATEGORIES,
                            'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=new_product') .
                            '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' .
                            tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $pInfo->
                            products_id . '&action=delete_product') . '">' . tep_image_button('button_delete.gif',
                            IMAGE_DELETE) . '</a> <a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' .
                            $cPath . '&pID=' . $pInfo->products_id . '&action=move_product') . '">' .
                            tep_image_button('button_move.gif', IMAGE_MOVE) . '</a> <a href="' .
                            tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $pInfo->
                            products_id . '&action=copy_to') . '">' . tep_image_button('button_copy_to.gif',
                            IMAGE_COPY_TO) . '</a>');
...
//MVS end
                    ...
                    $contents[] = array('text' => '<br>' . TEXT_PRODUCTS_PRICE_INFO . ' ' . $currencies->
                            format($pInfo->products_price) . '<br>' . TEXT_PRODUCTS_QUANTITY_INFO . ' ' . $pInfo->
                            products_quantity . '<br>Warehouse Quantity: ' . $pInfo->warehouse_quantity);
                ...
                $contents[] = array('text' =>
                       ...
                       Map eBay Category: <br><b>' . $ebay_category_name .
                        ...
                        '<span id="ebaycategory_1">' . tep_draw_pull_down_menu('ebaycategory_1', $ebay_categories,
                        '', 'style="width:100%;"') . '<br><br></span>' . '</div>', );
                //EOF:ebay_integration
				...
				$contents[] = array('text' => 'Map google Category: <br><b>' . $google_category_name .
                        ...
                        '<span id="googlecategory_1">' . tep_draw_pull_down_menu('googlecategory_1', $google_categories,
                        '', 'style="width:100%;"') . '<br><br></span>' . '</div><span id="mapgooglebutton" style="visibility:hidden;"><input type="button" id="mapgooglebutton" value="Map Google Category" style="width:100%;" /><br><br></span>', );
            } else
