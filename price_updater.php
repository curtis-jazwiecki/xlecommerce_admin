...
function tep_get_category_path($id, $from = 'category', $categories_array = '', $index = 0) {
    ...
    if ($from == 'product') {
      ...
      $index++;
      if ( (tep_not_null($category['parent_id'])) && ($category['parent_id'] != '0') ) $categories_array = tep_get_category_path($category['parent_id'], 'category', $categories_array, $index);
    }
 ...
 // Check to see if they want to save the price update for future price updates
	if($_POST['save_price_update'] == true)
	  { 
		...
		//BOF:mod
		*/
	    $update_string_query = 'INSERT INTO price_updates (above_below, above_below_value, above_below_2, above_below_value_2,price_update_add, price_update_fixed, price_update_value, price_update_category, price_update_manufacturer, price_update_select_query, price_update_query, price_update_roundoff, price_update_sort_order, customer_group_id) VALUES ('.$above_below.' , '.$above_below_value.', '.$above_below_2.', '.$above_below_value_2.', '.$add.', '.$fixed.', '.$value.', '.$cat.', '.$mfr.', "'.addslashes($insert_into_price_select).'", "'.addslashes($insert_into_price_update).'", '.(isset($_POST['chk_roundoff']) ? 1 : 0).', '.($num_rows_results['price_updates_id'] + 1).', ' . ($customer_group_id=='' ? "null" : "'" . $customer_group_id . "'") . ')';
		//EOF:mod
