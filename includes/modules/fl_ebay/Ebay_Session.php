...
{ 
  ...
  /**
   * Read accessor of RequestToken. 
   * Specifies a user's ID token making API calls. If given the token is used when possible and RequestUser/RequestPassword are ignored.
   * 
   * @access public 
   * @return string Value of the RequestToken property
   */
  function getRequestToken()
  {
    return $this->_props['RequestToken'];
  } 

  /**
   * Write accessor of RequestToken.  
   * Specifies a user's ID token making API calls. If given the token is used when possible and RequestUser/RequestPassword are ignored.
   * 
   * @access public 
   * @param string $value The new value for the RequestToken property
   * @return void 
   */
  function setRequestToken( $value )
  {
    $this->_props['RequestToken'] = $value;
  } 
   ...
   * use 0 for UTF-8 encoding (default), and 1 for ISO-88591
   * 
   * @access public 
   ...
   * normally when set to false working on UTF-8 will use the plain data returned from eBay. We have encountered that using UTF-8 with special characters in the data might result in an invalid XML returned from the back-end. You might switch to iso-8859-1 encoding (@see XmlEncoding) or use this flag to turn on an extra decoding of the returned data to UTF-8.
   * 
   * @access public 
  ...
  function InitFromConfig( $configFile )
  {
    ...
    if ( isset( $cfg['xml-encoding'] ) )
    {
      $this->setXmlEncoding( $cfg['xml-encoding'] );
    } 
    else
    {
  ...
  function setAppMode( $value )
  {
    ...
    switch ( $value )
    {
      ...
      case 2:
        ...
        break;
      case 10:
        $this->_setProp( 'ApiUrl', 'http://beta.sandbox.ebay.com/ws/api.dll' );
        $this->_setProp( 'AppId', $this->_keys['test'][0] );
        $this->_setProp( 'DevId', $this->_keys['test'][1] );
        $this->_setProp( 'CertId', $this->_keys['test'][2] );
        break;
      case 100:
        die( "no local mode yet" );
        break;
    } 
  } 
   ...
   * @return void 
   */
  function Ebay_Session( $configFile = null )
  { 
  ...
  function WriteTokenFile()
  {
    if ( $this->getTokenUsePickupFile() )
    {
      $fname = $this->getTokenPickupFile();
      $fh = fopen( $fname, "w+" );
      fwrite( $fh, $this->getRequestToken() );
      fclose( $fh );
    } 
  } 
} 
