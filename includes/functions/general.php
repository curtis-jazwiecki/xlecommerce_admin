...
function getAllTrackingDetails($order_id){
 ...
 function get_qpu_price($base_price, $markup, $roundoff) {
        ...
        if (empty($markup)) {
            ...
            if ($markup_in_margin) {
                ...
                $price = ($markup_in_negative ? ($base_price - ($markup_in_percent ? (($base_price * $markup_figure) / 100) : $markup_figure)) : ($base_price + ($markup_in_percent ? (($base_price * $markup_figure) / 100) : $markup_figure)));
            }
        }
   if ($roundoff == '1') {
        $pos = strpos($price, '.');
        ...
        if ($pos === false) {
            ...
            $price = $value_parts[0] . '.' . ($value_parts[1] + (99 - $value_parts[1]));
        }
    }
