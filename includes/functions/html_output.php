...
// Output a form pull down menu
  ...
  function tep_draw_multiselect_pull_down_menu($name, $values, $default = array(), $parameters = '', $required = false,$size = '10') {
    $field = '<select size="'.$size.'" multiple name="' . tep_output_string($name) . '"';
    ...
    for ($i=0, $n=sizeof($values); $i<$n; $i++) {
      $field .= '<option '.$values[$i]['param'].' value="' . tep_output_string($values[$i]['id']) . '"';
      if(!empty($default)){
...
function get_eBay_categories($parent_id = ''){
    $response = array();

    if (empty($parent_id)){
        $eBay_categories_query = tep_db_query("select category_id, category_name from ebay_categories where category_level='1' order by category_name");    
    } else {
        $eBay_categories_query = tep_db_query("select category_id, category_name from ebay_categories where category_level!=1 and parent_category_id='" . $parent_id . "'order by category_name");
    }
    if (tep_db_num_rows($eBay_categories_query)){
        $response[] = array('id' => '', 'text' => '-- Select --');
        while($entry = tep_db_fetch_array($eBay_categories_query)){
            $response[] = array(
...
function get_eBay_categories_html($parent_id, $parent_level){
    $html = '';
    $categories = get_eBay_categories($parent_id);
    if ($categories){
        $html = '<select name="ebaycategory_' . ($parent_level+1) . '" style="width:100%;">';
        foreach($categories as $category){
            $html .= '<option value="' . $category['id'] . '">' . $category['text'] . '</option>';            
        }
        $html .= '</select>'; 
        $html = '<span id="ebaycategory_' . ($parent_level+1) . '">' . $html . '<br><br></span>';
    }
    return $html;
}
//EOF:ebay_integration

function get_google_categories($parent_id = ''){
    $response = array();

    if (empty($parent_id)){
        $google_categories_query = tep_db_query("select category_id, category_name from google_categories where category_level='1' order by category_name");    
    } else {
        $google_categories_query = tep_db_query("select category_id, category_name from google_categories where category_level!=1 and parent_category_id='" . $parent_id . "'order by category_name");
    }
    if (tep_db_num_rows($google_categories_query)){
        $response[] = array('id' => '', 'text' => '-- Select --');
        while($entry = tep_db_fetch_array($google_categories_query)){
            $response[] = array(
                'id' => $entry['category_id'], 
                'text' => $entry['category_name']);
        }
...
function get_google_categories_html($parent_id, $parent_level){
    $html = '';
    $categories = get_google_categories($parent_id);
    if ($categories){
        $html = '<select name="googlecategory_' . ($parent_level+1) . '" style="width:100%;">';
        foreach($categories as $category){
            $html .= '<option value="' . $category['id'] . '">' . $category['text'] . '</option>';            
        }
        $html .= '</select>'; 
        $html = '<span id="googlecategory_' . ($parent_level+1) . '">' . $html . '<br><br></span>';
    }
    return $html;
}
