...
// check to see if php implemented session management functions - if not, include php3/php4 compatible session class
  if (!function_exists('session_start')) {
    ...
    define('PHP_SESSION_NAME', 'OBNAdminID');
...
//Header("Location: " . HTTPS_CATALOG_SERVER . "$REQUEST_URI");
//exit;
//}

//Admin begin
if (!isset($cron_script) || $cron_script != 'yes') {
   if (basename($PHP_SELF) != FILENAME_LOGIN 
   		&& basename($PHP_SELF) != FILENAME_PASSWORD_FORGOTTEN 
		&& basename($PHP_SELF) != FILENAME_FORBIDDEN 
		&& basename($PHP_SELF) != 'xml_feed_manager.php') {
		      tep_admin_check_login();
  } else {
 		  $check = preg_match('/^.+(.php){1}.+(.php){1}.*$/', $PHP_SELF, $matches);
		  if (count($matches)){
		      tep_admin_check_login();
		  }
          }
...
//Admin end
  ...
  if($HTTP_GET_VARS['search_drop_box'] == 'product')
    tep_redirect(tep_href_link('categories.php?search='.$HTTP_GET_VARS['search_text'], '', 'SSL'));
  elseif($HTTP_GET_VARS['search_drop_box'] == 'order_number')
    tep_redirect(tep_href_link('orders.php?oID='.$HTTP_GET_VARS['search_text'].'&action=edit', '', 'SSL'));
  elseif($HTTP_GET_VARS['search_drop_box'] == 'customer')
    tep_redirect(tep_href_link('customers.php?search='.$HTTP_GET_VARS['search_text'], '', 'SSL'));
?>