...
*************************************** */
	...
	function DrawTitle() {
		if ($this->use_ttf == 1 ) { 
			...
			$xpos = ($this->plot_area[0] + $this->plot_area_width / 2);
			$ypos = ImageFontHeight((int)$this->title_font); 
	...
	function SetMargins() {
		...
		if ($this->use_ttf == 1) {
			...
			$title_size = array(ImageFontWidth((int)$this->title_font) * strlen($this->title_txt),ImageFontHeight((int)$this->title_font));
			//$this->y_top_margin = ($title_size[1] * 4);
			if ($nbLines == 1) { 
				$this->y_top_margin = $title_size[1] * 4;
			} else { 
...
//		}
	...
	function SetXLabelHeight() {
		...
		if ($this->use_ttf == 1) {
			...
			if ($this->x_datalabel_angle == 90) {
				$this->x_label_height = $this->x_datalabel_maxlength * ImageFontWidth((int)$this->small_font) / 1.5;
			} else {
				$this->x_label_height = 5 * ImageFontHeight((int)$this->small_font);
			}
		}
	...
	function DrawXDataLabel($xlab,$xpos) {
		...
		//Draw an x data label centered at xlab
			if ($this->use_ttf) {
				...
				$xlab_size = array(ImageFontWidth((int)$this->axis_font)*StrLen($xlab), $this->small_font_height*3);
				if ($this->x_datalabel_angle == 90) {
					$y = $this->plot_area[3] + ImageFontWidth((int)$this->axis_font)*StrLen($xlab); //in pixels
					$x = $xpos - ($this->small_font_height);
					ImageStringUp($this->img, $this->axis_font,$x, $y ,$xlab, $this->ndx_text_color);
				} else {
					$y = $this->plot_area[3] + ImageFontHeight((int)$this->axis_font); //in pixels
					$x = $xpos - (ImageFontWidth((int)$this->axis_font)*StrLen($xlab))/2;
